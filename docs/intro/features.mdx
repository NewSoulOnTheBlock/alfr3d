---
title: Features
description: Alfr3d long-term memory, task planning, skills system, CLI commands, and browser tool in detail
---
...
On first launch, the Agent proactively asks the user for key information and records it in the workspace (default `~/alfr3d`) — including agent settings, user identity, and memory files.
...
<Frame>
  <img src="https://cdn.jsdelivr.net/gh/NewSoulOnTheBlock/alfr3d-assets@main/screenshots/en/web-console-evolution-demo.png" width="800" />
</Frame>
...
The Agent automatically organizes valuable information from conversations into knowledge pages, maintaining cross-references and indexes. The Web console provides document browsing and knowledge graph visualization. Knowledge is stored in `~/alfr3d/knowledge/` within the workspace.
...
- [Skill Hub](https://skills.alfr3d.local/): An open skill marketplace featuring official, community, and third-party skills. Install with one command.
...
- **Custom skills:** Created by users through conversation, stored in the workspace (`~/alfr3d/skills/`), capable of implementing any complex business process or third-party integration.

Install skills: `/skill install <name>` or `alfr3d skill install <name>`, supporting Skill Hub, GitHub, ClawHub, URL, and more.
...
Visit [skills.alfr3d.local](https://skills.alfr3d.local/) to browse all available skills, or use commands in conversation:
...
Alfr3d provides two command interaction methods, covering service management, skill installation, configuration, and more:

- **Terminal CLI:** Run `alfr3d <command>` in the system terminal, supporting `start`, `stop`, `restart`, `update`, `status`, `logs`, `skill`, etc.
...
alfr3d update             # Update and restart
alfr3d skill install pptx # Install a skill
alfr3d install-browser    # Install browser tool
```

See [Command Overview](https://docs.alfr3d.local/en/cli) for details.
