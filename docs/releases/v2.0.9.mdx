---
title: v2.0.9
description: Alfr3d 2.0.9 - Web Console model management, MCP protocol support, browser persistent login, new models and deployment hardening
---
...
Documentation: [Models Overview](https://docs.alfr3d.local/en/models)

<img width="720" alt="20260522113305" src="https://cdn.jsdelivr.net/gh/NewSoulOnTheBlock/alfr3d-assets@main/screenshots/en/web-console-models-config.png" />
...
- Compatible with the `mcpServers` configuration style used by Claude Desktop / Cursor, reads `~/alfr3d/mcp.json` by default

Documentation: [MCP Tools](https://docs.alfr3d.local/en/tools/mcp). Thanks [@yangluxin613](https://github.com/yangluxin613) (#2801)
...
- **Persistent user profile (default)**: Uses `~/.alfr3d/browser_profile` as the browser user data dir by default; once logged in, sessions are reused automatically on subsequent runs
- **CDP mode**: Configure `tools.browser.cdp_endpoint` to take over a real Chrome instance with full browser permissions

Documentation: [Browser Tool](https://docs.alfr3d.local/en/tools/browser). Thanks [@leafmove](https://github.com/leafmove) (#2809)
...
- **TTS rolls out to more channels**: Web Console, Personal WeChat, Feishu, DingTalk and WeCom Smart Bot all support voice replies — see the [Channels Overview](https://docs.alfr3d.local/en/channels)
...
Source-code deployments can run `alfr3d update` for a one-click upgrade, or pull the latest code and restart manually. See the [Upgrade Guide](https://docs.alfr3d.local/en/guide/upgrade) for details.

**Release Date**: 2026.05.22 | [Full Changelog](https://github.com/NewSoulOnTheBlock/alfr3d/compare/2.0.8...2.0.9)
