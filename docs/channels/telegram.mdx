---
title: Telegram
description: Integrate Alfr3d with Telegram via the Bot API
---

> Integrate Alfr3d into Telegram via the official Bot API. Supports private chat and group chat (triggered by @mention or replying to the bot). Uses Long Polling — no public IP required, works out of the box.
...
2. Send `/newbot` and follow the prompts:
   - **Bot name** (display name, e.g. `My Alfr3d Bot`)
   - **Bot username** (must end with `bot`, e.g. `my_alfr3d_bot`)
...
```
[Telegram] Bot logged in as @my_alfr3d_bot (id=123456789)
...
- **Private chat**: search for your bot username (e.g. `@my_alfr3d_bot`) in Telegram, click `Start` and chat away.
