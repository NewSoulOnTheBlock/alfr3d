---
title: WeChat
description: Connect Alfr3d to personal WeChat (via the official API)
---

> Connect Alfr3d to your personal WeChat — scan to log in, no public IP required. Supports text, image, voice, file, and video messages in 1-on-1 chats. Backed by WeChat's official API; safe to use. After connecting, a bot assistant is added to your conversation list without affecting normal account usage.

## 1. Setup and run

### Option A: Web console

Start Alfr3d and open the Web console (local URL: http://127.0.0.1:9899/). Go to the **Channels** tab, click **Connect Channel**, select **WeChat**, and follow the prompts to scan in.
...
After starting Alfr3d, a QR code is displayed in the terminal. Scan it with WeChat to complete login.
...
After a successful login, credentials are saved to `~/.weixin_alfr3d_credentials.json`. Subsequent startups reuse the saved credentials with no need to re-scan.

To force a re-login, delete the credentials file and restart.

### Session expiry

When the WeChat session expires (errcode `-14`), Alfr3d automatically clears old credentials and initiates a new QR login — no manual intervention required.
