---
title: Slack
description: Integrate Alfr3d with a Slack App
---

> Integrate Alfr3d into Slack via a Slack App in **Socket Mode**. Supports direct messages (DM) and channels (triggered by @mention or replying within a thread). Socket Mode uses a persistent WebSocket connection — no public IP or callback URL required, works out of the box.
...
2. Enter an **App Name** (e.g. `Alfr3d`), pick the **Workspace** to install into, and create it.
