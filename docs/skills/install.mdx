...
Alfr3d supports installing skills from [Alfr3d Skill Hub](https://skills.alfr3d.local/), GitHub, ClawHub, LinkAI, and any URL via a unified `install` command. Use `/skill install` in chat or `alfr3d skill install` in the terminal.

## From the Skill Hub

Browse all available skills at [skills.alfr3d.local](https://skills.alfr3d.local/) and install by name:
...
<Tip>
  All commands above work in the terminal by replacing `/skill` with `alfr3d skill`. See [Skill Management Commands](/cli/skill) for full documentation.
</Tip>
