...
| `metadata.alfr3d.requires.bins` | System CLI tools that must be installed |
| `metadata.alfr3d.requires.env` | Required environment variables (all must be present) |
| `metadata.alfr3d.requires.anyEnv` | Multiple API keys — at least one must be set |
| `metadata.alfr3d.requires.anyBins` | Multiple tools — at least one must be installed |
| `metadata.alfr3d.always` | Set to `true` to always load, skipping dependency checks |
| `metadata.alfr3d.emoji` | Display emoji (optional) |
| `metadata.alfr3d.os` | OS restriction, e.g. `["darwin", "linux"]` |
