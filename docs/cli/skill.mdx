...
Skill management commands are used to install, query, and manage Alfr3d skills. Use `/skill <subcommand>` in chat or `alfr3d skill <subcommand>` in the terminal.
...
```bash Terminal
alfr3d skill search pptx
```
</CodeGroup>

## install

Install skills with a single `install` command from Alfr3d Skill Hub, GitHub, ClawHub, or any URL (zip archives, SKILL.md links) — no manual download or configuration required.
...
```bash Terminal
# Install all skills in a repo (auto-discovers subdirectories with SKILL.md)
alfr3d skill install larksuite/cli

# Specify a subdirectory to install a single skill
alfr3d skill install https://github.com/larksuite/cli/tree/main/skills/lark-im

# Use # to specify a subdirectory
alfr3d skill install larksuite/cli#skills/lark-minutes
```
</CodeGroup>
...
```bash Terminal
alfr3d skill install clawhub:baidu-search
```
</CodeGroup>
...
```bash Terminal
# Install from a zip archive (single or batch)
alfr3d skill install https://cdn.link-ai.tech/skills/pptx.zip
...
```bash Terminal
alfr3d skill enable pptx
alfr3d skill disable pptx
```
</CodeGroup>
...
| `alfr3dhub` | Installed from Alfr3d Skill Hub |
