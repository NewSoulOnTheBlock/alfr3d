...
The following commands can be used in chat with the `/` prefix or in the terminal with the `alfr3d` prefix (some are chat-only).
...
Show the current Alfr3d version.
