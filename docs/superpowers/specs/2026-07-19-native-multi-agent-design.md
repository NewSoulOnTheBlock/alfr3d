...
Allow one Alfr3d process to host multiple agents without changing the
behaviour of existing single-agent installations. Each agent is a complete
Alfr3d workspace, not a lightweight persona overlay.
...
When `agents` is absent, Alfr3d synthesizes one `default` profile from the
...
- Existing configurations still use `~/alfr3d` exactly as before.
