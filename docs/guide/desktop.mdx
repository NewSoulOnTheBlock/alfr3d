...
description: Download and use the Alfr3d desktop client (macOS / Windows)
---

Alfr3d ships a ready-to-use desktop client with the Agent runtime bundled in — **no need to install Python or dependencies manually**. Just download, install, and run your local super AI assistant.
...
<Card title="Go to the download page" icon="download" href="https://alfr3d.local/download/">
  Download the macOS / Windows installer
</Card>

<Tabs>
  <Tab title="macOS">
    1. Open the [download page](https://alfr3d.local/download/) and pick the build for your chip:
       - Apple Silicon (M1/M2/M3/M4): download the `arm64` build
       - Intel: download the `x64` build
    2. Open the downloaded `.dmg` and drag Alfr3d into your Applications folder.
    3. Launch Alfr3d from Launchpad or Applications.
  </Tab>
  <Tab title="Windows">
    1. Open the [download page](https://alfr3d.local/download/) and download the Windows installer (`.exe`).
    2. Run the installer and follow the prompts (you can choose the install directory).
    3. Launch Alfr3d from the desktop or Start menu.
  </Tab>
</Tabs>
...
- **Config directory**: `~/.alfr3d` in your home folder, holding `config.json` (model keys, channels, etc.) along with logs, cache and other runtime data.
- **Workspace**: `~/alfr3d` by default, holding chat history, knowledge base, memory, skills, scheduled tasks and other files produced by the Agent.
