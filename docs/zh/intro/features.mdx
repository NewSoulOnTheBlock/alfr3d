---
title: 功能介绍
description: Alfr3d 长期记忆、个人知识库、任务规划、技能系统、CLI 命令、浏览器工具详细说明
---
...
第一次启动 Agent 时，Agent 会主动询问关键信息，并记录至工作空间（默认 `~/alfr3d`）中的智能体设定、用户身份、记忆文件中。
...
<Frame>
  <img src="https://cdn.jsdelivr.net/gh/NewSoulOnTheBlock/alfr3d-assets@main/screenshots/zh/web-console-evolution-demo-zh.png" width="800" />
</Frame>
...
Agent 会在对话中自动将有价值的信息整理为知识页面，维护交叉引用和索引，通过 Web 控制台可浏览文档和查看知识图谱。知识库存储在工作空间的 `~/alfr3d/knowledge/` 目录下。
...
内置 `browser` 工具，Agent 可控制浏览器访问网页、填写表单、点击元素、截图，支持动态 JS 渲染页面。运行 `alfr3d install-browser` 一键安装，自动适配服务器（无头模式）和桌面环境：
...
- [Skill Hub](https://skills.alfr3d.local/)：开放的技能广场，汇集官方推荐、社区贡献和第三方技能，支持一键安装。
- **内置技能：** 在项目的 `skills/` 目录下，包含技能创造器、图像识别、LinkAI 智能体、网页抓取等。内置 Skill 根据依赖条件（API Key、系统命令等）自动判断是否启用。
- **自定义技能：** 由用户通过对话创建，存放在工作空间中（`~/alfr3d/skills/`），可实现任何复杂的业务流程和第三方系统对接。

安装技能：`/skill install <名称>` 或 `alfr3d skill install <名称>`，支持从 Skill Hub、GitHub、ClawHub、URL 等来源安装。
...
访问 [skills.alfr3d.local](https://skills.alfr3d.local/) 浏览所有可用技能，或在对话中执行：
...
Alfr3d 提供两种命令交互方式，覆盖服务管理、技能安装、配置调整等日常运维操作：

- **终端 CLI：** 在系统终端执行 `alfr3d <命令>`，支持 `start`、`stop`、`restart`、`update`、`status`、`logs`、`skill` 等
...
alfr3d stop               # 停止服务
alfr3d update             # 更新并重启
alfr3d skill install pptx # 安装技能
alfr3d install-browser    # 安装浏览器工具
```

详细命令参考 [命令总览](https://docs.alfr3d.local/cli)。
