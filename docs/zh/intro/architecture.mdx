---
title: 项目架构
description: Alfr3d 2.0 的系统架构和核心设计
---

Alfr3d 2.0 从简单的聊天机器人全面升级为超级智能助理，采用 Agent 架构设计，具备自主思考、规划任务、长期记忆和技能扩展等能力。

## 系统架构

Alfr3d 的整体架构由以下核心模块组成：

<img src="https://cdn.jsdelivr.net/gh/NewSoulOnTheBlock/alfr3d-assets@main/architecture/zh/architecture.jpg" alt="Alfr3d Architecture" />
...
| **CLI** | 命令行系统，提供终端命令（`alfr3d`）和对话命令（`/`），支持进程管理、技能安装、配置修改、知识库管理等操作 |

## Agent 模式

启用 Agent 模式后，Alfr3d 会以自主智能体的方式运行，核心工作流如下：
...
Agent 的工作空间默认位于 `~/alfr3d` 目录，用于存储系统提示词、记忆文件、技能文件等：
...
| `alfr3d_lang` | 界面、命令文案、系统提示词等的语言，`auto` 自动检测，可设为 `zh` / `en` | `auto` |
