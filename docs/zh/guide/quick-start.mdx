---
title: 一键安装
description: 使用脚本一键安装和管理 Alfr3d
---
...
| `alfr3d stop` | 停止服务 |
| `alfr3d restart` | 重启服务 |
| `alfr3d status` | 查看运行状态 |
| `alfr3d logs` | 查看实时日志 |
| `alfr3d update` | 更新代码并重启 |
| `alfr3d install-browser` | 安装浏览器工具依赖 |

更多命令和用法参考 [命令文档](/zh/cli/index)。

<Note>
  如果 `alfr3d` 命令不可用，也可以使用 `./run.sh <命令>`（Linux/macOS）或 `.\scripts\run.ps1 <命令>`（Windows）作为替代，功能等效。
</Note>
