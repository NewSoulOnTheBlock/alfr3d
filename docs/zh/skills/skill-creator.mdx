...
| `metadata.alfr3d.requires.bins` | 系统里必须装了哪些命令行工具 |
| `metadata.alfr3d.requires.env` | 需要哪些环境变量（全部满足才行） |
| `metadata.alfr3d.requires.anyEnv` | 多个 API Key 满足一个就行 |
| `metadata.alfr3d.requires.anyBins` | 多个工具满足一个就行 |
| `metadata.alfr3d.always` | 设为 `true` 会始终加载，不检查依赖 |
| `metadata.alfr3d.emoji` | 展示用的 emoji（可选） |
| `metadata.alfr3d.os` | 限定系统，如 `["darwin", "linux"]` |
