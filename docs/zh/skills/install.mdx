...
Alfr3d 支持通过统一的 `install` 命令安装来自 [Alfr3d 技能广场](https://skills.alfr3d.local/)、GitHub、ClawHub、LinkAI 以及任意 URL 上的技能。在对话中使用 `/skill install`，在终端中使用 `alfr3d skill install`。

## 从Alfr3d技能广场安装

访问 [skills.alfr3d.local](https://skills.alfr3d.local/) 浏览所有可用技能，找到想要的技能后直接安装，例如：
...
<Tip>
  以上所有命令在终端中使用时，将 `/skill` 替换为 `alfr3d skill` 即可。完整命令说明参考 [技能管理命令](/zh/cli/skill)。
</Tip>
