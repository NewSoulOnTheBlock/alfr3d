...
- **采样参数**：思考模式下 `temperature`、`top_p`、`presence_penalty`、`frequency_penalty` 会被服务端忽略（不会报错），Alfr3d 会自动跳过传入。
- **多轮工具调用**：当历史中包含工具调用时，DeepSeek 要求所有 assistant 消息必须回传 `reasoning_content`。Alfr3d 会自动处理回传逻辑，跨轮次切换思考开关也不会出错。
