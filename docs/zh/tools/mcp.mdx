...
Alfr3d 支持 [Model Context Protocol (MCP)](https://modelcontextprotocol.io)，让 Agent 能够直接调用社区中数以万计的 MCP 工具。配置一次 `mcp.json`，工具就会以与内置工具完全相同的方式呈现给 LLM，可被自动选择和调用。

## 配置文件

Alfr3d 读取 `~/alfr3d/mcp.json`。文件不存在时不会启用任何 MCP 工具，也不会报错。

Docker 部署时，官方 `docker-compose.yml` 已经把宿主机 `./alfr3d` 挂载到容器内 `/home/agent/alfr3d`（即容器用户的 `~/alfr3d`），把 `mcp.json` 放进宿主机 `./alfr3d/` 目录即可生效。
...
Alfr3d 自带 `read` / `write` / `edit` 工具，**直接把要装的 MCP 配置发给 Agent，让它写到配置文件中：

例如：

```markdown
帮我把这个 MCP 加到 ~/alfr3d/mcp.json 里：
...
部分远程 MCP需要 OAuth 网页授权，直接配置会返回 `401`。Alfr3d 内置标准 OAuth 流程，**无需手填 token**，正常配置即可，例如：
...
- **凭证存储**：令牌持久化在 `~/.alfr3d/mcp_oauth.json`，重启后复用。
...
- **热更新**：用户或 Agent 修改 `mcp.json` 后，消息处理完成时会自动重载变更的 server，无需重启 alfr3d
...
| 现象 | 排查方向 |
| --- | --- |
| 启动后 Agent 没有 MCP 工具 | 检查 `~/alfr3d/mcp.json` 是否存在、JSON 格式是否合法 |
...
| Docker 部署 | 确认宿主机 `./alfr3d` 已挂载到容器内 `/home/agent/alfr3d`，`mcp.json` 直接放进宿主机 `./alfr3d/` 目录即可，或者直接对话 Agent 安装 |
...
只要遵循 MCP 标准协议（stdio / SSE / Streamable HTTP），都可以直接接入 Alfr3d。
