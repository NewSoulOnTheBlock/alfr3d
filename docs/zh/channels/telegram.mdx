---
title: Telegram
description: 将 Alfr3d 接入 Telegram Bot
---

> 通过 Telegram Bot API 接入 Alfr3d，支持单聊与群聊（@机器人 / 回复机器人触发），使用 Long Polling 模式无需公网 IP，开箱即用。
...
2. 发送 `/newbot` 命令，按提示输入：
   - **Bot 名称**（显示名，可中文，例如 `My Alfr3d Bot`）
   - **Bot 用户名**（必须以 `bot` 结尾，例如 `my_alfr3d_bot`）
...
- **单聊**：在 Telegram 中搜索你创建的 Bot 用户名（如 `@my_alfr3d_bot`），点击 `Start` 即可开始对话。
