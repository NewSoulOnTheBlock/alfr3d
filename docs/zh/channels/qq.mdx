---
title: QQ 机器人
description: 将 Alfr3d 接入 QQ 机器人（WebSocket 长连接模式）
---

> 通过 QQ 开放平台的机器人接口接入 Alfr3d，支持 QQ 单聊、QQ 群聊（@机器人）、频道消息和频道私信，无需公网 IP，使用 WebSocket 长连接模式。
...
启动 Alfr3d项目后打开 Web 控制台 (本地链接为: http://127.0.0.1:9899/ )，选择 **通道** 菜单，点击 **接入通道**，选择 **QQ 机器人**，填写上一步保存的 AppID 和 AppSecret，点击接入即可。
