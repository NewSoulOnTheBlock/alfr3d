---
title: 飞书
description: 将 Alfr3d 接入飞书应用
---

> 通过飞书自建应用接入 Alfr3d，支持单聊与群聊（@机器人），使用 WebSocket 长连接模式，无需公网 IP，支持流式打字机回复、语音消息收发。
...
启动 Alfr3d 项目后在终端中即可完成扫码创建。或打开 Web 控制台（本地链接：http://127.0.0.1:9899 ），选择 **通道** 菜单，点击 **接入通道**，选择 **飞书**，点击 **一键创建飞书应用**，使用 **飞书 App** 扫描二维码即可自动完成应用创建并接入:
...
1. 启动 Alfr3d 项目后，在飞书开放平台点击 **事件与回调**，选择 **长连接** 模式并保存：
