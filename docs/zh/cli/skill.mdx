...
技能管理命令用于安装、查询和管理 Alfr3d 的技能。在对话中使用 `/skill <子命令>`，在终端中使用 `alfr3d skill <子命令>`。
...
安装技能。通过统一的 `install` 命令，可一键安装来自 **Alfr3d 技能广场、GitHub、ClawHub** 以及任意 URL（zip 压缩包、SKILL.md 链接）上的技能，无需手动下载和配置。
...
| --- | --- |
| `builtin` | 项目内置技能 |
| `alfr3dhub` | 从 Alfr3d Skill Hub 安装 |
