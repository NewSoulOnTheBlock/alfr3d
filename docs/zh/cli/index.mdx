---
title: 命令总览
description: Alfr3d 命令系统 — 终端 CLI 和对话命令
---

Alfr3d 提供两种命令交互方式：

- **终端CLI** — 在系统终端中执行 `alfr3d <命令>`，用于服务管理、技能管理等运维操作
- **对话命令** — 在对话中输入 `/<命令>` 或 `alfr3d <命令>`，用于查看状态、管理技能、调整配置等

## 终端命令

通过一键安装脚本部署后，`alfr3d` 命令会自动可用。手动安装的用户需要在项目根目录下额外执行：

```bash
pip install -e .
```

安装后即可在任意位置使用 `alfr3d` 命令：
