...
Alfr3d は [Model Context Protocol (MCP)](https://modelcontextprotocol.io) をサポートしており、コミュニティで提供されている数万種類の MCP Tool を Agent から直接呼び出せます。`mcp.json` を一度設定すれば、組み込みToolとまったく同じ形で LLM に公開され、自動的に選択・呼び出されます。

## 設定ファイル

Alfr3d は `~/alfr3d/mcp.json` を読み込みます。ファイルが存在しない場合は MCP Tool は読み込まれず、エラーにもなりません。

Docker デプロイの場合、公式の `docker-compose.yml` はホスト側の `./alfr3d` をコンテナ内の `/home/agent/alfr3d`（コンテナユーザーの `~/alfr3d`）にマウント済みです。ホスト側の `./alfr3d/` に `mcp.json` を置くだけで反映されます。
...
Alfr3d には `read` / `write` / `edit` Tool が組み込まれているため、**MCP の設定をそのまま Agent に渡して、ファイルに書き込んでもらえます**：

例：

```markdown
この MCP を ~/alfr3d/mcp.json に追加してください：
...
一部のリモート MCP は OAuth の Web 認可が必要で、そのまま設定すると `401` が返ります。Alfr3d は標準的な OAuth フローを内蔵しているため、**token を手動で入力する必要はなく**、通常どおり設定するだけです。例：
...
- **認証情報の保存**：token は `~/.alfr3d/mcp_oauth.json` に永続化され、再起動後も再利用されます。
...
- **ホットリロード**：ユーザーまたは Agent が `mcp.json` を変更すると、メッセージ処理完了時に変更された server のみが自動でリロードされます。alfr3d の再起動は不要です
...
| 症状 | 確認ポイント |
| --- | --- |
| 起動後に MCP Tool が一つもない | `~/alfr3d/mcp.json` が存在し、JSON が正しいか確認 |
...
| Docker デプロイ | ホストの `./alfr3d` がコンテナ内の `/home/agent/alfr3d` にマウントされていることを確認し、ホスト側の `./alfr3d/` に `mcp.json` を配置。または Agent に直接インストールを依頼 |
...
MCP 標準プロトコル（stdio / SSE / Streamable HTTP）に準拠していれば、コードを一切変更せずに Alfr3d に統合できます。
