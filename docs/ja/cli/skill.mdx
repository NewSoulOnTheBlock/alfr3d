...
スキル管理コマンドは Alfr3d のスキルのインストール、検索、管理に使用します。チャットでは `/skill <サブコマンド>`、ターミナルでは `alfr3d skill <サブコマンド>` を使用します。
...
統一された `install` コマンドで、Alfr3d スキル広場、GitHub、ClawHub、任意の URL（zip アーカイブ、SKILL.md リンク）からスキルをワンクリックでインストールできます。手動ダウンロードや設定は不要です。
...
| --- | --- |
| `builtin` | プロジェクト内蔵スキル |
| `alfr3dhub` | Alfr3d Skill Hub からインストール |
