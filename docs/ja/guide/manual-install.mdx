---
title: 手動インストール
description: Alfr3dの手動デプロイ（ソースコード / Docker）
---
...
<Tip>
  ネットワークに問題がある場合は、ミラーを使用してください: https://gitee.com/NewSoulOnTheBlock/alfr3d
</Tip>
...
<Note>
  このステップは推奨です。インストール後、`alfr3d start`、`alfr3d stop`、`alfr3d update` でサービスを管理でき、`alfr3d skill` でスキルを管理できます。CLI をインストールしない場合は、`./run.sh` または `python3 app.py` で実行できます。
</Note>
...
| `agent_workspace` | Agent のワークスペースパス | `~/alfr3d` |
...
<Tip>
  すべての設定オプションはプロジェクトの [`config.py`](https://github.com/NewSoulOnTheBlock/alfr3d/blob/master/config.py) に記載されています。
</Tip>
