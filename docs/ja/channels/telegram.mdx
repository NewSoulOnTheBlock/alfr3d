---
title: Telegram
description: Telegram Bot API 経由で Alfr3d を接続
---

> 公式の Telegram Bot API を通じて Alfr3d を接続します。1 対 1 チャットおよびグループチャット（@メンションまたはボットへの返信で起動）に対応。Long Polling 方式のため公開 IP は不要で、すぐに利用できます。
...
2. `/newbot` を送り、案内に従って入力します:
   - **Bot 名**（表示名、例: `My Alfr3d Bot`）
   - **Bot ユーザー名**（`bot` で終わる必要があります、例: `my_alfr3d_bot`）
...
<Tabs>
  ...
  <Tab title="設定ファイル">
    `config.json` に以下を追加して Alfr3d を起動します:
...
- **1 対 1 チャット**: Telegram で Bot のユーザー名（例: `@my_alfr3d_bot`）を検索し、`Start` をタップして会話を開始します。
