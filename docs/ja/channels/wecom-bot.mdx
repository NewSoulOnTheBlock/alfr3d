---
title: WeCom Bot
description: Alfr3d を WeCom AI Bot に接続する（WebSocket ロングコネクション）
---

WeCom AI Bot を介して Alfr3d を接続し、ダイレクトメッセージとグループチャットの両方に対応します。パブリック IP は不要で、WebSocket ロングコネクションを使用し、Markdown レンダリングとストリーミング出力をサポートします。
