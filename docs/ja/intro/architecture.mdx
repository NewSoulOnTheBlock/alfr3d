---
title: アーキテクチャ
description: Alfr3d 2.0 のシステムアーキテクチャとコア設計
---

Alfr3d 2.0 は、シンプルなチャットボットから、自律的な思考、タスク計画、長期記憶、Skill の拡張性を備えた Agent アーキテクチャのスーパーインテリジェントアシスタントへと進化しました。

## システムアーキテクチャ

Alfr3d のアーキテクチャは以下のコアモジュールで構成されています：
...
| **CLI** | コマンドラインシステム。ターミナルコマンド（`alfr3d`）とチャットコマンド（`/`）で、プロセス管理、Skill インストール、設定変更、ナレッジベース管理などをサポート |

## Agent モードのワークフロー

Agent モードが有効な場合、Alfr3d は以下のワークフローで自律的な Agent として動作します：
...
Agent のワークスペースはデフォルトで `~/alfr3d` にあり、システムプロンプト、記憶ファイル、Skill ファイルを格納しています：
...
シークレットキーはセキュリティのため `~/.alfr3d` ディレクトリに別途保存されます：
...
| `agent_workspace` | ワークスペースのパス | `~/alfr3d` |
...
| `alfr3d_lang` | UI・コマンド文言・システムプロンプトなどの言語。`auto` で自動検出、`zh` / `en` も指定可 | `auto` |
