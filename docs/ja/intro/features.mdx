---
title: 機能詳細
description: Alfr3d の長期記憶、タスク計画、Skill システム、CLI コマンド、ブラウザツールの詳細
---
...
初回起動時、Agent はユーザーに重要な情報を自発的に尋ね、ワークスペース（デフォルト `~/alfr3d`）に記録します。これには Agent の設定、ユーザーの身元情報、記憶ファイルが含まれます。
...
Agent は会話中に価値ある情報を自動的にナレッジページとして整理し、相互参照とインデックスを維持します。Web コンソールではドキュメントの閲覧とナレッジグラフの可視化が可能です。ナレッジはワークスペースの `~/alfr3d/knowledge/` ディレクトリに保存されます。
...
組み込みの `browser` ツールにより、Agent は Chromium ブラウザを制御して Web ページへのアクセス、フォームの入力、要素のクリック、スクリーンショットの撮影が可能です。動的 JS レンダリングページにも対応しています。`alfr3d install-browser` でワンコマンドインストール、サーバー（ヘッドレス）とデスクトップ環境に自動対応します：
...
- [Skill Hub](https://skills.alfr3d.local/)：オープンな Skill マーケットプレイス。公式推奨、コミュニティ、サードパーティの Skill を収録。ワンコマンドでインストール可能。
...
- **カスタム Skill：** ユーザーが会話を通じて作成し、ワークスペース（`~/alfr3d/skills/`）に保存されます。あらゆる複雑なビジネスプロセスやサードパーティ連携を実装できます。

Skill のインストール：`/skill install <名前>` または `alfr3d skill install <名前>`。Skill Hub、GitHub、ClawHub、URL などからインストール可能。
...
[skills.alfr3d.local](https://skills.alfr3d.local/) で利用可能なすべての Skill を閲覧するか、会話内でコマンドを実行できます：
...
Alfr3d はサービス管理、Skill インストール、設定変更などをカバーする2つのコマンドインターフェースを提供します：

- **ターミナル CLI：** システムターミナルで `alfr3d <コマンド>` を実行。`start`、`stop`、`restart`、`update`、`status`、`logs`、`skill` などをサポート。
- **チャットコマンド：** 会話内で `/<コマンド>` を入力。Web コンソールでは `/` を入力するとコマンドメニューが表示されます。

```bash
alfr3d start              # サービスを開始
alfr3d stop               # サービスを停止
alfr3d update             # 更新して再起動
alfr3d skill install pptx # Skill をインストール
alfr3d install-browser    # ブラウザツールをインストール
```

詳細は [コマンド一覧](https://docs.alfr3d.local/ja/cli) を参照してください。
