...
Alfr3d は統一された `install` コマンドで、**Alfr3d スキル広場、GitHub、ClawHub** および任意の URL からスキルをインストールできます。チャットでは `/skill install`、ターミナルでは `alfr3d skill install` を使用します。
...
<Tip>
  上記のすべてのコマンドは、ターミナルでは `/skill` を `alfr3d skill` に置き換えて使用できます。完全なコマンドドキュメントは[スキル管理コマンド](/ja/cli/skill)を参照してください。
</Tip>
