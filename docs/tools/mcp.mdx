...
Alfr3d supports the [Model Context Protocol (MCP)](https://modelcontextprotocol.io), allowing the Agent to directly invoke tens of thousands of community MCP tools. Configure `mcp.json` once and the tools are exposed to the LLM in exactly the same way as built-in tools — automatically selected and invoked.

## Configuration File

Alfr3d reads `~/alfr3d/mcp.json`. If the file does not exist, no MCP tools are loaded — and no error is raised.

For Docker deployments, the official `docker-compose.yml` already mounts the host's `./alfr3d` directory to `/home/agent/alfr3d` inside the container (i.e. the container user's `~/alfr3d`). Just drop `mcp.json` into the host's `./alfr3d/` directory and it will take effect.
...
Alfr3d ships with `read` / `write` / `edit` tools, so **you can simply send the MCP config to the Agent and ask it to write the file**:

For example:

```markdown
Add this MCP to ~/alfr3d/mcp.json:
...
Some remote MCP servers require OAuth web authorization, and connecting to them directly returns `401`. Alfr3d has a built-in standard OAuth flow, so **no manual token is needed** — just configure the server normally, for example:
...
- **Credential storage**: Tokens are persisted in `~/.alfr3d/mcp_oauth.json` and reused across restarts.
...
- **Hot reload**: When you or the Agent modifies `mcp.json`, changed servers are automatically reloaded after the current message — no need to restart alfr3d.
...
| --- | --- |
| Agent has no MCP tools after startup | Verify that `~/alfr3d/mcp.json` exists and contains valid JSON |
...
| Docker deployment | Make sure host's `./alfr3d` is mounted to `/home/agent/alfr3d` in the container, then just drop `mcp.json` into host's `./alfr3d/`. Or just ask the Agent to do it |
...
Any MCP server that follows the standard protocol (stdio / SSE / Streamable HTTP) integrates with Alfr3d out of the box.
