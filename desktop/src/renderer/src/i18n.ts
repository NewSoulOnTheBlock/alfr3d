const translations: Record<string, Record<string, string>> = {
  zh: {
    ...
    menu_feedback: '反馈',
    // onboarding
    onboarding_welcome_title: '欢迎使用 Alfr3d',
    ...
    channels_empty_desc: '点击右上角「接入通道」按钮，即可将 Alfr3d 接入微信、飞书、钉钉等消息通道',
    ...
    status_error: '初始化失败',
    status_error_desc: '后端服务未能启动，客户端无法连接',
    status_error_hint: '若重试无效，请打开日志目录查看 run.log，反馈其中的报错信息，或尝试使用管理员权限启动客户端。',
    status_retry: '重试',
  ...
  en: {
    ...
    // onboarding
    onboarding_welcome_title: 'Welcome to Alfr3d',
    ...
    channels_empty_desc: 'Click "Add channel" above to connect Alfr3d to WeChat, Feishu, DingTalk and more',
    ...
    status_error_desc: 'The backend service did not start, so the client cannot connect',
    status_error_hint: 'If retrying does not help, open the log folder and check run.log, report the error it shows, or try starting the client with administrator privileges.',
    status_retry: 'Retry',
...
// in alfr3d_lang and always wins. Falls back to 'en' when the locale is unknown.
function detectDefaultLang(): Lang {
  const locale = (window.electronAPI?.systemLocale || navigator.language || '').toLowerCase()
  return locale.startsWith('zh') ? 'zh' : 'en'
}

const savedLang = localStorage.getItem('alfr3d_lang') as Lang | null
...
export function setLang(lang: Lang) {
  currentLang = lang
  localStorage.setItem('alfr3d_lang', lang)
}
