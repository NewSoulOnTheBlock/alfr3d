...
const ALFR3D_DATA_DIR = path.join(os.homedir(), '.alfr3d')
...
export class PythonBackend extends EventEmitter {
  ...
  getDataDir(): string {
    return this.findBundledBackend() ? ALFR3D_DATA_DIR : this.backendPath
  }
  ...
  private findBundledBackend(): string | null {
    const exeName = process.platform === 'win32' ? 'alfr3d-backend.exe' : 'alfr3d-backend'
    const candidates = [
      path.join(this.backendPath, 'alfr3d-backend', exeName),
   ...
   * packaged build keeps config in ALFR3D_DATA_DIR (~/.alfr3d); dev reads it from the
   ...
   * (ALFR3D_WEB_PORT) and the renderer (whenPortReady / the 'port' event).
   */
  ...
  async start(): Promise<void> {
    ...
    // Packaged app stores writable data in ~/.alfr3d; dev keeps it in the repo.
    const dataDir = bundled ? ALFR3D_DATA_DIR : this.backendPath
    ...
    if (bundled) {
      command = bundled
      args = []
      // Run from the writable data dir (~/.alfr3d), NOT the install dir. When the
      ...
      // to point elsewhere.
      try {
        fs.mkdirSync(ALFR3D_DATA_DIR, { recursive: true })
      } catch {
    ...
    this.process = spawn(command, args, {
      cwd,
      // ALFR3D_DESKTOP enables the lighter desktop runtime (no plugins, no MCP).
      // ALFR3D_DATA_DIR (packaged only) redirects writable data to ~/.alfr3d so the
      ...
      env: {
        ...
        ALFR3D_WEB_PORT: String(this.port),
        ...(bundled ? { ALFR3D_DATA_DIR } : {}),
      },
