...
//   2. User themes — ~/.alfr3d/themes/, the shared data dir users can add to
...
function alfr3dRoot(): string {
  // Honor an explicit override, else default to ~/.alfr3d to match the backend.
  return process.env.ALFR3D_HOME || path.join(os.homedir(), '.alfr3d')
}

export function themesDir(): string {
  return path.join(alfr3dRoot(), THEMES_DIRNAME)
}
...
// (~/.alfr3d/themes). Bundled themes take precedence on id conflicts so a shipped
