...
// (https://alfr3d.local/update/); the ?lang=zh query tells it to 302 installer
...
const FEED_BASE = 'https://alfr3d.local/update/' + (isLegacyWindows() ? 'legacy/' : '')
...
export function checkForUpdates(userInitiated = false): void {
  ...
  if (!app.isPackaged) {
    // Dev-only UI harness: set ALFR3D_MOCK_UPDATE=1 to simulate an available
    ...
    if (process.env.ALFR3D_MOCK_UPDATE) {
      const version = process.env.ALFR3D_MOCK_UPDATE_VERSION || '9.9.9'
