...
"""
PyInstaller spec for the Alfr3d desktop backend (onedir).

Produces a self-contained `alfr3d-backend` folder that the Electron app
...
Build from the repo root:
    pyinstaller desktop/build/alfr3d-backend.spec --noconfirm
"""
import os
...
# NewSoulOnTheBlock/alfr3d#2987: "客户端没有飞书通道").
hiddenimports = [
...
# thread) so command plugins like alfr3d_cli/godcmd (/status, #help) work. Plugin
# modules are imported dynamically by name in scan_plugins(), so list them
# explicitly. The `cli` package is a alfr3d_cli dependency (`from cli import ...`).
hiddenimports += [
    'plugins',
    'plugins.event',
    'plugins.plugin',
...
# `cli` powers alfr3d_cli's slash commands (`alfr3d skill install`, `alfr3d status`, …).
...
# download) or downloaded on demand into ~/.alfr3d/ms-playwright at first use.
...
# desktop/build/build-feishu-vendor.py) into ~/.alfr3d/feishu_vendor and adds it to
# sys.path. This keeps the base download small while Feishu still works out of
# the box after a one-time fetch (fixes NewSoulOnTheBlock/alfr3d#2987).
excludes = [
    ...
    # is downloaded on demand into ~/.alfr3d/ms-playwright. See browser_env.py.
]
