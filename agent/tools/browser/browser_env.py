...
def get_data_root() -> str:
    """Writable data root (~/.alfr3d on desktop, else CWD-based).

    Mirrors the logic in common/log.py without importing config, to avoid a
    circular import. The desktop build sets ALFR3D_DATA_DIR; source deployments
...
def browsers_download_dir() -> str:
    ...
    We pin it under the writable data root (~/.alfr3d/ms-playwright) rather than
...
def is_desktop() -> bool:
    ...
    The desktop shell always sets ALFR3D_DESKTOP=1 (see python-manager.ts), both in
    `npm run dev` (runs app.py with the user's Python) and in the packaged build
    (frozen exe). Desktop users have no `alfr3d` CLI, so onboarding must point them
    at the in-chat `/install-browser` command rather than a terminal command.
    """
    return os.environ.get("ALFR3D_DESKTOP") == "1"
