...
remote MCP servers that guard their endpoint behind OAuth (e.g. Xmind):
  ...
  4. Token exchange + refresh, persisted to ~/.alfr3d/mcp_oauth.json.
...
# ------------------------------------------------------------------
# Token store: ~/.alfr3d/mcp_oauth.json  {server_name: {...credentials...}}
# ------------------------------------------------------------------
...
def _store_path() -> str:
    base = os.path.expanduser("~/.alfr3d")
    try:
...
class OAuthHandler:
    ...
    def __init__(self, server_name: str, resource_url: str, redirect_uri: str,
                 scope: str = "", client_name: str = "Alfr3d"):
