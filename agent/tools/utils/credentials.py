...
The agent's API keys live in ~/.alfr3d/.env and must only ever be reached through
...
narrowed to ~/.alfr3d/.env. See also issue #2913 for the bypasses handled here.
"""
...
# loaded from ~/.alfr3d/.env). Reading them bypasses the env_config boundary.
...
def is_credential_path(absolute_path: str) -> bool:
    ...
    Beyond the literal ~/.alfr3d/.env file, this also blocks two real bypass
    surfaces reported in issue #2913:
      1. /proc/<pid|self|thread-self>/environ - a second view of the
         process environment that leaks secrets loaded from ~/.alfr3d/.env.
      2. Symlinks resolving to ~/.alfr3d/.env; an exact abspath match keeps the
    ...
    env_real = os.path.realpath(expand_path("~/.alfr3d/.env")).replace(os.sep, "/")
