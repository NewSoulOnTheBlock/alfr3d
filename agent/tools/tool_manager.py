...
class ToolManager:
    ...
    def _read_mcp_json_signature(self):
        """
        Return (mtime, sha256_of_bytes) for ~/alfr3d/mcp.json without parsing.
    ...
    def _load_mcp_configs(self) -> list:
        ...
        Load MCP server configs with priority:
          1. ~/alfr3d/mcp.json  (supports both mcpServers and mcp_servers keys)
    ...
    def refresh_mcp_if_changed(self):
        """
        Cheap check whether ~/alfr3d/mcp.json has changed since last load.
    ...
    def sync_mcp_into_agent(self, agent) -> tuple:
        ...
        Handles both representations Alfr3d uses:
