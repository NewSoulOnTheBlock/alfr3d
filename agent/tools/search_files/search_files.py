...
# mirrors any secrets loaded into the process environment from ~/.alfr3d/.env.
...
class SearchFiles(BaseTool):
    ...
    def __init__(self, config: dict = None):
        ...
        # Resolved once (assumes ~/.alfr3d is stable for this session), not per-check.
        self._alfr3d_dir = os.path.realpath(expand_path("~/.alfr3d")).replace(os.sep, "/")
    ...
    def _is_credential_path(self, absolute_path: str) -> bool:
        """Block ~/.alfr3d (like ls.py) and /proc/*/environ (like read.py #2913)."""
        candidates = set()
        try:
        ...
        for candidate in candidates:
            if _PROC_ENVIRON_RE.match(candidate):
                return True
        return any(c == self._alfr3d_dir or c.startswith(self._alfr3d_dir + "/") for c in candidates)
