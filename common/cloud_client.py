...
class CloudClient(LinkAIClient):
    ...
    def on_chat(self, data: dict, send_chunk_fn):
        ...
        with _acting_user(user_id):
            # Intercept alfr3d/slash commands before the agent runs
            try:
                from plugins import PluginManager
                mgr = PluginManager()
                instance = mgr.instances.get("ALFR3D_CLI")
                ...
                logger.warning(f"[CloudClient] alfr3d_cli intercept failed: {e}")
...
ALFR3D_SEND_WEB_SUBDIR = "alfr3d-send"


def copy_send_file(src_path: str, workspace_root: str) -> str:
    """Copy *src_path* into ``websites/alfr3d-send/`` and return its URL.
    ...
    send_dir = os.path.join(ws, "websites", ALFR3D_SEND_WEB_SUBDIR)
    try:
    ...
    try:
        ...
        return ""
    return f"{base}/{ALFR3D_SEND_WEB_SUBDIR}/{dest_name}"
