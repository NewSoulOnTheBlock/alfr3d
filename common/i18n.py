...
Resolution priority (highest first):
  1. Explicit `alfr3d_lang` from config.json — also covers Docker/CI, since any
     config key is overridable via its uppercase env var (e.g. ALFR3D_LANG=zh),
     handled by config.load_config() before resolution. ALFR3D_LANG is a private
...
def _detect_from_env():
    ...
    The alfr3d_lang env override (ALFR3D_LANG=zh) is intentionally NOT read here:
    it sets config["alfr3d_lang"] and is handled via the explicit config path,
    not auto-detection.
    """
...
def detect_language():
    ...
    Order (auto-detection only; explicit config["alfr3d_lang"] is resolved
...
def resolve_language(configured=None):
    ...
    `configured` is the raw `alfr3d_lang` value from config.json (may be None,
