"""
Alfr3dCli plugin - Intercept alfr3d/slash commands in chat messages.
...
# Known top-level subcommands that alfr3d supports.
...
@plugins.register(
    name="alfr3d_cli",
    desc="Handle alfr3d/slash commands in chat messages",
    version="0.1.0",
    author="Alfr3d",
    desire_priority=1000,
)
class Alfr3dCliPlugin(Plugin):

    def __init__(self):
        super().__init__()
        self.handlers[Event.ON_HANDLE_CONTEXT] = self.on_handle_context
        self.aliases = self._build_aliases()
        logger.debug("[Alfr3dCli] initialized")
    ...
    def _build_aliases() -> dict:
        ...
        try:
            ...
            logger.warning(f"[Alfr3dCli] could not read command_aliases from config: {e}")
            overrides = {}
        if not isinstance(overrides, dict):
            logger.warning(f"[Alfr3dCli] command_aliases must be an object, got {type(overrides).__name__}; ignoring")
            overrides = {}
        for key, value in overrides.items():
            if not isinstance(key, str) or not isinstance(value, str):
                logger.warning(f"[Alfr3dCli] ignoring non-string alias entry: {key!r} -> {value!r}")
                continue
            k = key.strip().lower()
            if k:
        ...
        for key, value in merged.items():
            ...
            if head not in KNOWN_COMMANDS:
                logger.warning(f"[Alfr3dCli] dropping alias '/{key}' -> '{value}': unknown command '{head}'")
                continue
            if key in KNOWN_COMMANDS:
                logger.warning(f"[Alfr3dCli] alias '/{key}' shadows a real command; the command takes precedence")
            valid[key] = value
        return valid
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        if kind == "run":
            _, cmd, args = result
            logger.info(f"[Alfr3dCli] intercepted command: {cmd} {args}")
    ...
    def _parse_command(self, content: str):
        """
        Parse alfr3d command from message text.

        Supported formats:
          alfr3d <command> [args...]   e.g. "alfr3d skill list"
        ...
        if content.startswith("alfr3d "):
            ...
            if cmd not in KNOWN_COMMANDS:
                # 'alfr3d xxx' that isn't a command — don't intercept (could be
                # natural language like "alfr3d xxx 怎么样").
                return None
            args = parts[1] if len(parts) > 1 else ""
            return cmd, args
    ...
    def _resolve(self, token: str, user_args: str):
        ...
        is trivially unit-testable. Note the `alfr3d ` form never reaches stages
    ...
    def execute(self, query: str, session_id: str = "") -> str:
        """Execute a alfr3d/slash command string without a channel context.
    ...
    def _dispatch(self, cmd: str, args: str, e_context: EventContext, session_id: str = "") -> str:
        if cmd in CLI_ONLY_COMMANDS:
            return _t(
                f"⚠️ `alfr3d {cmd}` 只能在命令行终端中执行。\n请在终端运行: alfr3d {cmd}",
                f"⚠️ `alfr3d {cmd}` can only run in a terminal.\nRun it in your shell: alfr3d {cmd}",
            )
        ...
        if handler:
            try:
                ...
                logger.error(f"[Alfr3dCli] command '{cmd}' failed: {e}")
    ...
    def _cmd_help(self, args: str, e_context, **_) -> str:
        if _t("zh", "en") == "en":
            lines = [
                ...
                "",
                "💡 You can also use alfr3d <command> instead of /<command>",
            ]
        else:
            lines = [
                "📋 Alfr3d 命令列表",
                ...
                "",
                "💡 也可以用 alfr3d <command> 代替 /<command>",
            ]
        return "\n".join(lines)

    def _cmd_version(self, args: str, e_context, **_) -> str:
        return f"Alfr3d v{__version__}"
    ...
    def _cmd_status(self, args: str, e_context: EventContext, session_id: str = "", **_) -> str:
        ...
        cfg = conf()
        lines = [_t("📊 Alfr3d 运行状态", "📊 Alfr3d Status"), ""]
    ...
    def _config_set(self, key: str, value_str: str) -> str:
        ...
        # Write to the data dir (~/.alfr3d in desktop, project root otherwise), not
        ...
        logger.info(f"[Alfr3dCli] config update: {updates}, synced envs: {synced_envs}")

        try:
            load_config()
        except Exception as e:
            logger.warning(f"[Alfr3dCli] config reload warning: {e}")
    ...
    # ------------------------------------------------------------------
    # install-browser (shared logic with alfr3d install-browser CLI)
    # ------------------------------------------------------------------
    ...
    def _send_install_progress(e_context, text: str) -> None:
        ...
        if e_context is None:
            logger.info(f"[Alfr3dCli] install-browser: {text}")
            return
        try:
            ...
            logger.warning(f"[Alfr3dCli] install-browser progress send failed: {e}")
    ...
    def _cmd_install_browser(self, args: str, e_context, **_) -> str:
        ...
        if args.strip():
            return _t(
                "用法: /install-browser\n\n"
                "无需参数，等同于终端执行 `alfr3d install-browser`。\n"
                "安装过程可能持续数分钟；进度会以多条消息推送，pip 详细输出见服务日志。",
                "Usage: /install-browser\n\n"
                "No arguments needed; equivalent to running `alfr3d install-browser` in a terminal.\n"
        ...
        if code != 0:
            return _t(
                ...
                "也可在终端执行 `alfr3d install-browser`。",
                "❌ Installation did not finish successfully. Check the messages above or the server log; "
                "you can also run `alfr3d install-browser` in a terminal.",
            )
        return _t(
            "✅ 安装流程已结束。请重启 Alfr3d 后使用 browser 工具。",
            "✅ Installation finished. Restart Alfr3d to use the browser tool.",
        )
    ...
    def _refresh_skill_manager(self):
        ...
        try:
            ...
            logger.debug(f"[Alfr3dCli] skill refresh skipped: {e}")
    ...
    def _skill_list_remote(self, page: int = 1) -> str:
        ...
        lines.append(_t("🌐 https://skills.alfr3d.local  在线浏览全部技能", "🌐 https://skills.alfr3d.local  Browse all skills online"))
        return "\n".join(lines)
    ...
    def _memory_dream(self, days: int, e_context, session_id: str) -> str:
        ...
        def _run():
            try:
                ...
                logger.warning(f"[Alfr3dCli] /memory dream failed: {e}")
    ...
    def _memory_dream_sync(self, flush_mgr, days: int) -> str:
        ...
        try:
            ...
            logger.warning(f"[Alfr3dCli] /memory dream sync failed: {e}")
    ...
    def _memory_rebuild_index(self, e_context, session_id: str) -> str:
        ...
        try:
            ...
            logger.exception("[Alfr3dCli] /memory rebuild-index: build provider failed")
        ...
        def _run():
            try:
                ...
                logger.exception("[Alfr3dCli] /memory rebuild-index failed")
    ...
    def _memory_rebuild_sync(memory_manager, model_label, dim_label) -> str:
        ...
        try:
            ...
            logger.exception("[Alfr3dCli] /memory rebuild-index sync failed")
    ...
    def _notify(e_context, text: str):
        ...
        try:
            ...
            logger.warning(f"[Alfr3dCli] notify failed: {e}")
    ...
    def _knowledge_toggle(self, enabled: bool) -> str:
        ...
        # Data dir (~/.alfr3d in desktop), not __file__-relative — see _config_set.
        from config import get_data_root
        config_path = os.path.join(get_data_root(), "config.json")
        try:
            with open(config_path, "r", encoding="utf-8") as f:
    ...
    def get_help_text(self, **kwargs):
        return _t("在对话中使用 /help 或 alfr3d help 查看可用命令", "Use /help or alfr3d help in chat to see available commands")
