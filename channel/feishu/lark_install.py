...
the desktop build (see NewSoulOnTheBlock/alfr3d#2987 — "客户端没有飞书通道").
...
VENDOR_URLS = (
    ...
    "https://cdn.alfr3d.local/desktop/vendor/feishu-vendor-{version}.zip",
)

DOWNLOAD_TIMEOUT = 120

# Per-user, writable, persistent location. Mirrors the browser tool's ~/.alfr3d
# layout so everything Alfr3d owns lives under one roof.
_VENDOR_SUBDIR = os.path.join(".alfr3d", "feishu_vendor")
...
def _fetch(url: str) -> bytes:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Alfr3d"})
