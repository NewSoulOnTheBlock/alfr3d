...
Implementation note:
    ...
    with its own asyncio loop so the rest of alfr3d (which is sync) stays
    untouched. Inbound messages are dispatched onto alfr3d's existing sync
...
class DiscordChannel(ChatChannel):
    ...
    def send(self, reply: Reply, context: Context):
        """Called from alfr3d's sync main thread; marshal the coroutine onto the loop thread."""
