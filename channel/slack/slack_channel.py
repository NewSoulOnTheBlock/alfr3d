...
Implementation note:
    ...
    threads. We start it in a dedicated thread so the rest of alfr3d (sync) stays
    untouched. Inbound events are dispatched onto alfr3d's existing sync
...
class SlackChannel(ChatChannel):
    ...
    def send(self, reply: Reply, context: Context):
        """Called from alfr3d's sync main thread; Slack Web client is sync-safe."""
