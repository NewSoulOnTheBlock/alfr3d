...
Implementation note:
    ...
    thread with its own asyncio loop so the rest of alfr3d (which is sync)
    stays untouched. Inbound updates are dispatched onto alfr3d's existing
...
class TelegramChannel(ChatChannel):
    ...
    def startup(self):
        ...
        # Run the asyncio event loop in a dedicated thread so the sync alfr3d body
        # is untouched.
    ...
    async def _on_message(self, update, _context):
        ...
        try:
            ...
            # Dispatch to alfr3d main pipeline (reuses ChatChannel._compose_context routing)
    ...
    def send(self, reply: Reply, context: Context):
        """Called from alfr3d's sync main thread; we marshal the coroutine onto the loop thread."""
