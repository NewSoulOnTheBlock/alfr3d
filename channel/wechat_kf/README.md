...
> 与 `channel/wechatcom/`（企微自建应用）是两个**独立的 Alfr3d 通道**：
>
...
```
┌─────────────────────┐    ┌─────────────────────┐    ┌──────────────────┐
│ 1. 企业微信后台      │ →  │ 2. Alfr3d 配置回调      │ →  │ 3. 绑定微信客服   │
│   创建一个自建应用   │    │   端口 9888          │    │   账号           │
└─────────────────────┘    └─────────────────────┘    └──────────────────┘
                                                              ↓
                                                   外部微信用户通过
...
回到企微后台「设置API接收」点击保存——会触发 `GET /wxkf/?...&echostr=...`，Alfr3d 通过 `crypto.check_signature` 校验后返回明文 `echostr`，验证成功。
