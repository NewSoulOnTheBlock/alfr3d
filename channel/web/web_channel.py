...
def _log_bind_failure(host: str, port: int, err: OSError):
    ...
    if winerror == 10013:
        ...
        logger.error(
            f"[WebChannel] 端口 {port} 已被占用，可执行 `alfr3d restart` 清理残留进程，"
...
def _check_auth():
    ...
    if not _is_password_enabled():
        return True
    if _verify_auth_token(web.cookies().get("alfr3d_auth_token", "")):
        return True
...
def _require_auth():
    ...
    if not _check_auth():
        ...
        offered = []
        if web.cookies().get("alfr3d_auth_token", ""):
...
class WebChannel(ChatChannel):
    ...
    def chat_page(self):
        ...
        # it on first load (when the user has no saved alfr3d_lang preference).
        return html.replace("{{ALFR3D_DEFAULT_LANG}}", i18n.get_language())

    def startup(self):
        configured_host = conf().get("web_host", "")
        host = configured_host or ("0.0.0.0" if _is_password_enabled() else "127.0.0.1")
        # The desktop app passes its chosen port via ALFR3D_WEB_PORT so its backend
        ...
        port = int(os.environ.get("ALFR3D_WEB_PORT") or conf().get("web_port", 9899))
        ...
        def _log_startup_banner():
            ...
            if os.environ.get("ALFR3D_DESKTOP") != "1":
                try:
                    import webbrowser
...
class AuthLoginHandler:
    def POST(self):
        ...
        web.setcookie("alfr3d_auth_token", token, expires=_session_expire_seconds(),
...
class AuthLogoutHandler:
    def POST(self):
        web.header('Content-Type', 'application/json; charset=utf-8')
        web.setcookie("alfr3d_auth_token", "", expires=-1, path="/")
        return json.dumps({"status": "success"})
...
class ChatHandler:
    def GET(self):
        ...
        html = html.replace("{{ALFR3D_DEFAULT_LANG}}", i18n.get_language())
        return html
...
class ConfigHandler:
    ...
    def GET(self):
        ...
        try:
            ...
            title = "Alfr3d" if use_agent else "AI Assistant"
    ...
    def POST(self):
        ...
        try:
            ...
            if "alfr3d_lang" in applied:
                try:
                    i18n.resolve_language(applied["alfr3d_lang"])
...
class ModelsHandler:
    ...
    def _set_embedding(self, provider_id: str, model: str) -> str:
        ...
        # the running MemoryManager (see plugins/alfr3d_cli). The dim may have
