/* =====================================================================
   Alfr3d Console - Main Application Script
   ===================================================================== */
...
// (alfr3d_lang) -> browser language -> 'zh'. Shares __alfr3dResolveLang__ defined in
// chat.html; falls back to a local resolver if loaded standalone.
let currentLang = (typeof window.__alfr3dResolveLang__ === 'function')
    ? window.__alfr3dResolveLang__()
    : (function () {
        const norm = (raw) => {
            if (!raw) return '';
        ...
        return norm(localStorage.getItem('alfr3d_lang'))
            || norm(window.__ALFR3D_DEFAULT_LANG__)
...
function applyI18n() {
    ...
    if (docsLink) docsLink.href = currentLang === 'zh' ? 'https://docs.alfr3d.local/zh' : 'https://docs.alfr3d.local';
...
// the backend `alfr3d_lang` config so logs / agent replies / CLI follow suit.
function setLanguage(lang) {
    ...
    currentLang = next;
    localStorage.setItem('alfr3d_lang', currentLang);
    applyI18n();
    _applyInputTooltips();
...
// Persist the language to the backend `alfr3d_lang` config (best-effort; the UI
...
function syncLanguageToBackend(lang, callback) {
    try {
        fetch('/config', {
            ...
            body: JSON.stringify({ updates: { alfr3d_lang: lang } })
        })
...
// Theme
// =====================================================================
let currentTheme = localStorage.getItem('alfr3d_theme') || 'dark';
...
function toggleTheme() {
    ...
    localStorage.setItem('alfr3d_theme', currentTheme);
    applyTheme();
}
...
function _openImageLightbox(src) {
    let overlay = document.getElementById('alfr3d-lightbox');
    if (!overlay) {
        overlay = document.createElement('div');
        overlay.id = 'alfr3d-lightbox';
    ...
    overlay.querySelector('#alfr3d-lightbox-img').src = src;
...
let appConfig = { use_agent: false, title: 'Alfr3d', subtitle: '', providers: {}, api_bases: {} };

const SESSION_ID_KEY = 'alfr3d_session_id';
...
function startSSE(requestId, loadingEl, timestamp, titleInfo, replayItems) {
    ...
    function ensureBotEl() {
        ...
        botEl.innerHTML = `
            <img src="assets/logo.jpg" alt="Alfr3d" class="w-8 h-8 rounded-lg flex-shrink-0">
    ...
    function processSSEItem(item) {
            if (item.type === 'reasoning') {
                ...
                // Coarse progress (e.g. alfr3d install-browser); must not close SSE (unlike "done")
                ensureBotEl();
...
function newChat(optimistic = true) {
    ...
    ws.innerHTML = `
        <img src="assets/logo.jpg" alt="Alfr3d" class="w-16 h-16 rounded-2xl mb-6 shadow-lg shadow-primary-500/20">
        <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-3">${appConfig.title || 'Alfr3d'}</h1>
...
const SESSION_PANEL_KEY = 'alfr3d_session_panel_open';
...
function initApp() {
    ...
    fetch('/api/version').then(r => r.json()).then(data => {
        ...
        document.getElementById('sidebar-version').textContent = `Alfr3d ${APP_VERSION}`;
    }).catch(() => {
        document.getElementById('sidebar-version').textContent = 'Alfr3d';
    });
    chatInput.focus();
}
