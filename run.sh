...
# Clone project
clone_project() {
    echo -e "${GREEN}🔍 Cloning Alfr3d project...${NC}"

    if [ -d "Alfr3d" ]; then
        # An existing directory is automatically backed up (no prompt) so the
        # installer stays one-shot / hands-off.
        local backup_dir="Alfr3d_backup_$(date +%s)"
        echo -e "${YELLOW}⚠️  $(t "目录 'Alfr3d' 已存在，自动备份到" "Directory 'Alfr3d' exists, backing up to") '$backup_dir'...${NC}"
        mv Alfr3d "$backup_dir"
    fi
    ...
    if ! command -v git &> /dev/null; then
        ...
        local zip_url="https://gitee.com/NewSoulOnTheBlock/alfr3d/repository/archive/master.zip"
        if command -v wget &> /dev/null; then
            wget "$zip_url" -O Alfr3d.zip
        elif command -v curl &> /dev/null; then
            curl -L "$zip_url" -o Alfr3d.zip
        else
        ...
        if command -v unzip &> /dev/null; then
            ...
            python3 -m zipfile -e Alfr3d.zip .
        elif command -v python &> /dev/null; then
            python -m zipfile -e Alfr3d.zip .
        else
            echo -e "${RED}❌ Cannot extract archive. Please install 'unzip' or Python.${NC}"
            exit 1
        fi
        # Archive top-level dir name may vary (Alfr3d-master, etc.); detect it.
        local _extracted="Alfr3d-master"
        if [ ! -d "$_extracted" ]; then
            _extracted=$(ls -d Alfr3d-*/ 2>/dev/null | head -1 | sed 's:/*$::')
        fi
        [ -n "$_extracted" ] && [ -d "$_extracted" ] && mv "$_extracted" Alfr3d
        rm -f Alfr3d.zip
    else
        ...
        if curl -sI --connect-timeout 5 --max-time 10 https://github.com > /dev/null 2>&1; then
            ...
            _timeout 60 git clone --depth 10 --progress https://github.com/NewSoulOnTheBlock/alfr3d.git && clone_ok=true
        fi
        if [ "$clone_ok" = false ]; then
            echo -e "${YELLOW}⚠️  GitHub clone failed or timed out, switching to Gitee mirror...${NC}"
            _timeout 30 git clone --depth 10 --progress https://gitee.com/NewSoulOnTheBlock/alfr3d.git && clone_ok=true
        fi
        if [ "$clone_ok" = false ]; then
    ...
    cd Alfr3d || { echo -e "${RED}❌ Failed to enter project directory.${NC}"; exit 1; }
    export BASE_DIR=$(pwd)
...
install_dependencies() {
    ...
    # Register `alfr3d` CLI command via editable install
    echo -e "${YELLOW}Registering alfr3d CLI...${NC}"
    set +e
    $PYTHON_CMD -m pip install -e . $PIP_EXTRA_ARGS $PIP_MIRROR > /dev/null 2>&1
    if command -v alfr3d &> /dev/null; then
        echo -e "${GREEN}✅ alfr3d CLI registered.${NC}"
    else
        echo -e "${YELLOW}⚠️  alfr3d CLI not in PATH, you can still use: $PYTHON_CMD -m cli.cli${NC}"
    fi
    set -e
}
...
base = {
    ...
    'alfr3d_lang': e('ALFR3D_LANG', 'auto'),
...
start_project() {
    echo ""
    echo -e "${GREEN}${EMOJI_ROCKET} Starting Alfr3d...${NC}"
    sleep 1

    local USE_ALFR3D=false
    if command -v alfr3d &> /dev/null; then
        USE_ALFR3D=true
    fi
    ...
    if $USE_ALFR3D; then
        ...
        if [[ "$OS_TYPE" == "Linux" ]]; then
            ...
            echo -e "${GREEN}${EMOJI_ALFR3D} Alfr3d started on Linux (using $PYTHON_CMD)${NC}"
        elif [[ "$OS_TYPE" == "Darwin" ]]; then
            nohup $PYTHON_CMD "${BASE_DIR}/app.py" > "${BASE_DIR}/nohup.out" 2>&1 &
            echo -e "${GREEN}${EMOJI_ALFR3D} Alfr3d started on macOS (using $PYTHON_CMD)${NC}"
        else
    ...
    echo ""
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${GREEN}${EMOJI_CHECK} $(t "Alfr3d 已在后台运行" "Alfr3d is now running in background")!${NC}"
    ...
    if $USE_ALFR3D; then
        echo -e "  ${GREEN}alfr3d stop${NC}       $(t "停止服务" "Stop the service")"
        echo -e "  ${GREEN}alfr3d restart${NC}    $(t "重启服务" "Restart the service")"
        echo -e "  ${GREEN}alfr3d status${NC}     $(t "查看状态" "Check status")"
        echo -e "  ${GREEN}alfr3d logs${NC}       $(t "查看日志" "View logs")"
        echo -e "  ${GREEN}alfr3d update${NC}     $(t "更新并重启" "Update and restart")"
        echo -e "  ${GREEN}alfr3d install-browser${NC}  $(t "安装浏览器工具" "Install browser tool")"
    else
    ...
    echo ""
    echo -e "${YELLOW}$(t "提示：需要让 Agent 浏览网页时，运行 alfr3d install-browser 安装浏览器工具" "Tip: to let the Agent browse the web, run 'alfr3d install-browser' to install the browser tool")${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
...
show_usage() {
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_ALFR3D} Alfr3d Management Script${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
...
# Check if alfr3d CLI is available
has_alfr3d() {
    command -v alfr3d &> /dev/null
}
...
cmd_start() {
    ...
    if has_alfr3d; then
        ...
        if is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} $(t "Alfr3d 已在运行中" "Alfr3d is already running") (PID: $(get_pid))${NC}"
...
cmd_stop() {
    ...
    if has_alfr3d; then
        ...
        echo -e "${GREEN}${EMOJI_STOP} $(t "正在停止 Alfr3d" "Stopping Alfr3d")...${NC}"

        if ! is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} $(t "Alfr3d 未在运行" "Alfr3d is not running")${NC}"
            return 0
        fi
        ...
        echo -e "${GREEN}${EMOJI_CHECK} $(t "Alfr3d 已停止" "Alfr3d stopped")${NC}"
    fi
}
...
cmd_status() {
    if has_alfr3d; then
        ...
        echo -e "${CYAN}${BOLD}=========================================${NC}"
        echo -e "${CYAN}${BOLD}   ${EMOJI_ALFR3D} Alfr3d Status${NC}"
        echo -e "${CYAN}${BOLD}=========================================${NC}"
...
cmd_config() {
    ...
    select_language
    echo ""
    echo -e "${YELLOW}${EMOJI_WRENCH} $(t "正在重新配置 Alfr3d" "Reconfiguring Alfr3d")...${NC}"
...
# Update project
cmd_update() {
    echo -e "${GREEN}${EMOJI_WRENCH} $(t "正在更新 Alfr3d" "Updating Alfr3d")...${NC}"
    cd "${BASE_DIR}"
    ...
    if [ -d .git ]; then
        ...
        if git pull; then
            ...
            git remote set-url origin https://gitee.com/NewSoulOnTheBlock/alfr3d.git
            if git pull; then
                pull_ok=true
            else
...
install_mode() {
    ...
    clear
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_ALFR3D} Alfr3d Installation${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
...
# Initialize UI_LANG for management commands: prefer alfr3d_lang from an existing
...
init_ui_lang() {
    ...
    local cfg_lang=""
    if [ -f "${BASE_DIR}/config.json" ]; then
        # `|| true`: grep returns 1 when alfr3d_lang is absent, which would abort
        # the whole script under `set -e` at the very first management command.
        cfg_lang=$(grep -o '"alfr3d_lang"[[:space:]]*:[[:space:]]*"[^"]*"' "${BASE_DIR}/config.json" 2>/dev/null | cut -d'"' -f4 || true)
    fi
    case "$cfg_lang" in
        zh) UI_LANG="zh" ;;
        en) UI_LANG="en" ;;
