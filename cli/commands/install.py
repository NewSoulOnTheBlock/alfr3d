"""alfr3d install-browser - Install Playwright + Chromium for the browser tool."""
...
def run_install_browser(
    ...
    Reused by ``alfr3d install-browser`` CLI and chat ``/install-browser``.
    ...
    stream("")
    stream("Browser tool ready! Restart Alfr3d to enable it.", "green")
    _phase(on_phase, _t(
        "🎉 全部步骤结束。请重启 Alfr3d 后使用 browser 工具。",
        "🎉 All steps finished. Restart Alfr3d to use the browser tool.",
    ))
    return 0
