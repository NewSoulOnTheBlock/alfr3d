"""Portable local backup and restore commands for Alfr3d user data."""
...
def _data_root() -> Path:
    configured = os.environ.get("ALFR3D_DATA_DIR")
...
def _workspace_from_config(config: dict) -> Path:
    return Path(config.get("agent_workspace") or "~/alfr3d").expanduser().resolve()
...
def create_backup_archive(
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="alfr3d-backup-"))
...
def _validate_archive(archive: zipfile.ZipFile) -> dict:
    ...
    if manifest.get("format") != BACKUP_FORMAT or manifest.get("version") != BACKUP_VERSION:
        raise ValueError("unsupported Alfr3d backup format or version")
...
def restore_backup_archive(
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="alfr3d-restore-"))
    try:
        ...
        if workspace is not None:
            ...
            target_workspace = Path("~/alfr3d").expanduser().resolve()
...
@click.option(
    ...
    help="Output .zip path (default: ./alfr3d-backup-<timestamp>.zip).",
)
def backup_command(output: Optional[Path]):
    ...
    if output is None:
        ...
        output = Path.cwd() / f"alfr3d-backup-{stamp}.zip"
...
def restore_command(archive: Path, workspace: Optional[Path], yes: bool):
    ...
    if pid:
        raise click.ClickException(
            f"Alfr3d is running (PID: {pid}). Run 'alfr3d stop' before restoring."
        )
    if not yes:
        click.confirm(
    ...
    if has_current_data:
        stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        rollback = archive.resolve().parent / f"alfr3d-pre-restore-{stamp}.zip"
