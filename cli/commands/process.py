"""alfr3d start/stop/restart/status/logs - Process management commands."""
...
def _get_pid_file():
    return os.path.join(get_project_root(), ".alfr3d.pid")
...
def start(foreground, no_logs):
    ...
    pid = _read_pid()
    if pid:
        click.echo(f"Alfr3d is already running (PID: {pid}).")
        return
    ...
    if foreground:
        click.echo("Starting Alfr3d in foreground...")
        if _IS_WIN:
        ...
        _write_pid(proc.pid)
        click.echo(click.style(f"✓ Alfr3d started (PID: {proc.pid})", fg="green"))
...
def stop():
    ...
    pid = _read_pid()
    if not pid:
        click.echo("Alfr3d is not running.")
        return

    click.echo(f"Stopping Alfr3d (PID: {pid})...")
    try:
        _kill_pid(pid)
        for _ in range(30):
            time.sleep(0.1)
    ...
    _remove_pid()
    click.echo(click.style("✓ Alfr3d stopped.", fg="green"))
...
def self_restart():
    ...
    own code), not by users — so it is hidden from `alfr3d help`. Unlike `restart`,
    ...
    """
    if _IS_WIN:
        click.echo("self-restart is not supported on Windows; use `alfr3d restart`.", err=True)
        sys.exit(1)
...
def update(ctx):
    ...
    if _IS_WIN:
        # On Windows, `alfr3d.exe` (this process) locks the exe file, so
        # `pip install -e .` fails with WinError 5.  Write a small .bat
        # helper that waits for alfr3d.exe to exit, then installs & starts.
        bat = os.path.join(root, "_alfr3d_update.bat")
        lines = [
            "@echo off",
            "chcp 65001 >nul",
            "echo Waiting for alfr3d.exe to exit...",
        ...
        lines += [
            "echo Reinstalling alfr3d CLI...",
            f'"{python}" -m pip install -e . -q',
            "echo Starting Alfr3d...",
        ...
        subprocess.Popen(
            ["cmd.exe", "/c", "start", "Alfr3d Update", "/wait", bat],
            cwd=root,
        )
        click.echo(click.style(
        ...
        click.echo("Reinstalling alfr3d CLI...")
        subprocess.call(
...
def status():
    ...
    # ModuleNotFoundError when `alfr3d` runs from outside the project dir.
    ...
    pid = _read_pid()
    if pid:
        click.echo(click.style(f"● Alfr3d is running (PID: {pid})", fg="green"))
    else:
        click.echo(click.style("● Alfr3d is not running", fg="red"))

    click.echo(_t(f"  版本: v{__version__}", f"  Version: v{__version__}"))

    # Project path bound to this `alfr3d` CLI — disambiguates which checkout the
