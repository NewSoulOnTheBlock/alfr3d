"""alfr3d skill - Skill management commands."""
...
def _clone_repo(git_url: str):
    ...
    """
    tmp_dir = tempfile.mkdtemp(prefix="alfr3d-skill-")
...
def _register_installed_skill(name: str, source: str = "alfr3dhub", display_name: str = ""):
    """Register a newly installed skill into skills_config.json.

    source values: builtin, alfr3d, github, clawhub, linkai, local, url
    """
    skills_dir = get_skills_dir()
...
def _print_install_success(name: str, source: str):
    ...
    # so it works when `alfr3d` is invoked from outside the project directory.
    get_cli_language()  # resolve alfr3d_lang so i18n.t reflects config
...
def _list_remote(page: int = 1):
    ...
    nav_parts = []
    if page > 1:
        nav_parts.append(f"alfr3d skill list --remote --page {page - 1}")
    if page < total_pages:
        nav_parts.append(f"alfr3d skill list --remote --page {page + 1}")
    if nav_parts:
        click.echo(f"  Navigate: {' | '.join(nav_parts)}")
    click.echo(f"  Install:  alfr3d skill install <name>")
    click.echo(f"  Browse:   https://skills.alfr3d.local\n")
...
def search(query):
    ...
    click.echo(f"\n  Install with: alfr3d skill install <name>\n")
...
# ------------------------------------------------------------------
# alfr3d skill install (CLI thin wrapper)
# ------------------------------------------------------------------
@skill.command()
@click.argument("name")
def install(name):
    ...
    Examples:
      ...
      alfr3d skill install pptx                          (from Skill Hub)

      alfr3d skill install larksuite/cli                 (GitHub shorthand, all skills)

      alfr3d skill install larksuite/cli#skills/lark-im  (single skill by subpath)

      alfr3d skill install https://github.com/owner/repo

      alfr3d skill install https://gitlab.com/org/repo

      alfr3d skill install git@github.com:owner/repo.git

      alfr3d skill install ./my-local-skills             (local directory)

      alfr3d skill install https://example.com/path/to/SKILL.md
    """
...
def _install_hub(name, result: InstallResult, provider=None):
    ...
    if "application/json" in content_type:
        ...
        if source_type == "github":
            ...
            _install_zip_bytes(mirror_resp.content, name, skills_dir, result=result, source_label="alfr3dhub", display_name=hub_display_name)
            if len(result.installed) == installed_before:
                _register_installed_skill(name, source="alfr3dhub", display_name=hub_display_name)
                result.installed.append(name)
        ...
        installed_before = len(result.installed)
        _install_zip_bytes(resp.content, name, skills_dir, result=result, source_label="alfr3dhub")
        if len(result.installed) == installed_before:
