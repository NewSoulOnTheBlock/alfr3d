...
available_setting = {
    ...
    "weixin_credentials_path": "~/.weixin_alfr3d_credentials.json",  # credentials file path
    ...
    "agent_workspace": "~/alfr3d",  # agent workspace path, used to store skills, memory, etc.
    # Optional native multi-agent registry. When empty or omitted, Alfr3d
    # synthesizes one "default" agent from agent_workspace and behaves exactly
    # as before. Each configured workspace is a complete Alfr3d workspace.
    "agents": [],
...
def load_config():
    ...
    # the desktop build points ALFR3D_DATA_DIR at ~/.alfr3d so config survives updates.
    ...
    resolved_lang = i18n.resolve_language(config.get("alfr3d_lang", "auto"))
...
def get_data_root():
    ...
    The desktop build sets ALFR3D_DATA_DIR (e.g. ~/.alfr3d) so data lives in the
...
def get_weixin_credentials_path():
    ...
    packaged desktop build (ALFR3D_DATA_DIR set) keeps it under the data dir
    (~/.alfr3d) so all user data stays together, while source deployments retain
    the legacy ~/.weixin_alfr3d_credentials.json default unchanged.
    """
    configured = conf().get("weixin_credentials_path")
    if configured:
        return os.path.expanduser(configured)
    if os.environ.get("ALFR3D_DATA_DIR"):
        return os.path.join(get_data_root(), "weixin_credentials.json")
    return os.path.expanduser("~/.weixin_alfr3d_credentials.json")
