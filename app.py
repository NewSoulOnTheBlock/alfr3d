...
# loaded in a background thread (so command plugins like alfr3d_cli/godcmd work
# without slowing startup), while MCP warmup is still skipped to keep it fast.
DESKTOP_MODE = os.environ.get("ALFR3D_DESKTOP") == "1"
        ...
        with self._lock:
            ...
            if first_start:
                if DESKTOP_MODE:
                    ...
                    # (alfr3d_cli / godcmd, e.g. /status, #help) work in the
...
def _warn_if_legacy_workspace_data_exists():
    """
    Warn if the hardcoded ~/alfr3d default holds data that agent_workspace
    ...
    try:
        ...
        legacy_root = expand_path("~/alfr3d")
