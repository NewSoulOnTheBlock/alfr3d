...
an unguarded edit leaked the contents of ~/.alfr3d/.env straight into the model's
...
class _TempHomeCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.makedirs(os.path.join(self.tmp, ".alfr3d"))
        self.env_path = os.path.join(self.tmp, ".alfr3d", ".env")
        with open(self.env_path, "w", encoding="utf-8") as f:
            f.write(f"OPENAI_API_KEY={_SECRET}\nANTHROPIC_KEY=sk-ant-other\n")

        self.workspace = os.path.join(self.tmp, "alfr3d")
...
class TestEditCredentialGuard(_TempHomeCase):
    ...
    def test_tilde_path_blocked(self):
        result = Edit(self.config).execute(
            {"path": "~/.alfr3d/.env", "oldText": "", "newText": "x\n"}
        )
        self.assertEqual(result.status, "error")
        self.assertIn(_DENIED, str(result.result))
...
class TestSharedGuard(_TempHomeCase):
    ...
    def test_ordinary_paths_not_blocked(self):
        ...
        self.assertFalse(is_credential_path(os.path.join(self.tmp, ".alfr3d", "config.json")))
