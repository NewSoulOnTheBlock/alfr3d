...
Verifies that the read tool blocks not just the literal ~/.alfr3d/.env file but
...
class TestReadCredentialBypass(unittest.TestCase):
    ...
    def test_direct_env_file_blocked(self):
        """Direct ~/.alfr3d/.env access must remain blocked (regression control)."""
        env_path = expand_path("~/.alfr3d/.env")
    ...
    def test_symlink_to_env_file_blocked(self):
        """A symlink resolving to ~/.alfr3d/.env must be blocked."""
        env_path = expand_path("~/.alfr3d/.env")
