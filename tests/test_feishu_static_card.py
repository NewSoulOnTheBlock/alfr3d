...
def test_plain_text_keeps_native_feishu_text_message():
    msg_type, content = build_text_delivery("hello from Alfr3d")

    assert msg_type == "text"
    assert json.loads(content) == {"text": "hello from Alfr3d"}
...
def test_markdown_detection_avoids_common_plain_text_punctuation():
    ...
    assert contains_markdown("Use `alfr3d --help` for details") is True
