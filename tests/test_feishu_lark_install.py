...
def offline(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ALFR3D_DATA_DIR", str(tmp_path))
    monkeypatch.delenv("ALFR3D_DESKTOP", raising=False)
...
def test_vendor_urls_prefer_the_china_mirror(monkeypatch):
    monkeypatch.delenv("ALFR3D_FEISHU_VENDOR_URL", raising=False)
...
def test_vendor_url_can_be_overridden(monkeypatch):
    monkeypatch.setenv("ALFR3D_FEISHU_VENDOR_URL", "https://example.test/b.zip")
...
def test_ensure_allow_install_false_raises(offline, monkeypatch):
    monkeypatch.setenv("ALFR3D_DESKTOP", "1")
    with pytest.raises(ImportError):
