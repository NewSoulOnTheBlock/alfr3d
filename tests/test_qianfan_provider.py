...
class TestQianfanConstantsAndRouting(unittest.TestCase):
    ...
    def test_alfr3d_cli_routes_ernie_models_to_qianfan(self):
        from common import const
        import plugins

        old_plugin_path = plugins.instance.current_plugin_path
        alfr3d_cli_was_registered = "ALFR3D_CLI" in plugins.instance.plugins
        old_alfr3d_cli_plugin = plugins.instance.plugins.get("ALFR3D_CLI")
        parent_had_alfr3d_cli = hasattr(plugins, "alfr3d_cli")
        old_parent_alfr3d_cli = getattr(plugins, "alfr3d_cli", None)
        module_names = ("plugins.alfr3d_cli", "plugins.alfr3d_cli.alfr3d_cli")
        old_modules = {
        ...
        plugins.instance.current_plugin_path = os.path.join(
            os.path.dirname(__file__), "..", "plugins", "alfr3d_cli"
        )
        try:
            import plugins.alfr3d_cli.alfr3d_cli
            alfr3d_cli_plugin = plugins.instance.plugins["ALFR3D_CLI"]
        finally:
            plugins.instance.current_plugin_path = old_plugin_path
            if alfr3d_cli_was_registered:
                plugins.instance.plugins["ALFR3D_CLI"] = old_alfr3d_cli_plugin
            else:
                plugins.instance.plugins.pop("ALFR3D_CLI", None)
            for name in module_names:
                if name in old_modules:
                    sys.modules[name] = old_modules[name]
                else:
                    sys.modules.pop(name, None)
            if parent_had_alfr3d_cli:
                plugins.alfr3d_cli = old_parent_alfr3d_cli
            elif hasattr(plugins, "alfr3d_cli"):
                delattr(plugins, "alfr3d_cli")

        self.assertEqual(
            alfr3d_cli_plugin._resolve_bot_type_for_model("ernie-4.5-turbo-128k"),
            const.QIANFAN,
        )
        self.assertEqual(
            alfr3d_cli_plugin._resolve_bot_type_for_model("qianfan"),
            const.QIANFAN,
        )
