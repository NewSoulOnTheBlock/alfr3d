...
class TestMemoryGlobalConfigSync(unittest.TestCase):
    def setUp(self):
        ...
        # run can't leave a stale ~/alfr3d-pointed one behind.
    ...
    def test_setup_memory_system_syncs_global_config(self):
        ...
        self.assertEqual(
            ...
            "agent_workspace after agent init, not the hardcoded ~/alfr3d default",
        )
    ...
    def test_conversation_store_shares_the_configured_workspace(self):
        ...
        self.assertTrue(
            ...
            f"the configured workspace {self.workspace}, not ~/alfr3d",
        )
    ...
    def test_conversation_store_honors_workspace_without_any_priming(self):
        ...
        self.assertTrue(
            ...
            f"before the first agent init, not ~/alfr3d",
        )

    def test_falls_back_to_alfr3d_when_agent_workspace_is_unset(self):
        """
        ...
        self.assertEqual(
            ...
            os.path.realpath(expand_path("~/alfr3d")),
            "an unset agent_workspace should still resolve to the ~/alfr3d default",
        )
...
class TestLegacyWorkspaceWarning(unittest.TestCase):
    ...
    never moves or touches data, only logs when the hardcoded `~/alfr3d`
    ...
    `~/alfr3d` on the machine running the test.
    """
    ...
    def setUp(self):
        ...
        self.legacy_root = os.path.join(self.tmp, "alfr3d")
    ...
    def test_no_warning_when_paths_differ_only_by_case(self):
        """
        ~/alfr3d and ~/ALFR3D are the same directory on a case-insensitive
