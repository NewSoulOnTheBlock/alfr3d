...
def test_credential_directory_is_blocked(tmp_path):
    ...
    # depend on ~/.alfr3d actually existing) live in the same test.
    tool = _make_tool(tmp_path)
    alfr3d_dir = expand_path("~/.alfr3d")
    assert tool._is_credential_path(alfr3d_dir) is True
    assert tool._is_credential_path(alfr3d_dir + "/some/nested/file.db") is True
    assert tool._is_credential_path(str(tmp_path)) is False

    result = tool.execute({"pattern": ".", "path": alfr3d_dir})
    assert result.status == "error"
    assert "Access denied" in str(result.result)


def test_credential_directory_is_pruned_mid_walk(tmp_path, monkeypatch):
    # A broad search rooted above ~/.alfr3d (not directly targeting it) must
    # still prune it during traversal rather than walking into it. Points
    # expand_path("~/.alfr3d") at a fake dir under tmp_path so this exercises
    # the real _is_credential_path logic without touching the real home dir.
    import agent.tools.search_files.search_files as sf_module
    fake_alfr3d = tmp_path / ".alfr3d"
    fake_alfr3d.mkdir()
    (fake_alfr3d / "secret.env").write_text("API_KEY=leaked\n", encoding="utf-8")
    monkeypatch.setattr(sf_module, "expand_path", lambda p: str(fake_alfr3d) if p == "~/.alfr3d" else p)
...
def test_symlink_to_credential_file_is_skipped_not_opened(tmp_path, monkeypatch):
    ...
    import agent.tools.search_files.search_files as sf_module
    fake_alfr3d = tmp_path / "fake_alfr3d"
    fake_alfr3d.mkdir()
    (fake_alfr3d / "secret.env").write_text("API_KEY=super-secret-value\n", encoding="utf-8")
    monkeypatch.setattr(sf_module, "expand_path", lambda p: str(fake_alfr3d) if p == "~/.alfr3d" else p)

    workspace = tmp_path / "workspace"
    workspace.mkdir()
    (workspace / "decoy.txt").symlink_to(fake_alfr3d / "secret.env")
...
def test_symlinked_directory_pointing_at_credential_dir_is_pruned(tmp_path, monkeypatch):
    ...
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    (workspace / "decoy_dir").symlink_to(fake_alfr3d, target_is_directory=True)
