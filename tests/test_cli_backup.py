"""Tests for portable Alfr3d backup archives."""
...
def test_backup_restore_round_trip(tmp_path):
    ...
    archive = tmp_path / "alfr3d-backup.zip"
...
def test_restore_rejects_path_traversal(tmp_path):
    ...
    manifest = {"format": "alfr3d-backup", "version": 1}
...
def test_fresh_restore_ignores_archive_controlled_destinations(tmp_path, monkeypatch):
    ...
    assert result["workspace"] == str((fake_home / "alfr3d").resolve())
    assert (fake_home / "alfr3d" / "MEMORY.md").exists()
