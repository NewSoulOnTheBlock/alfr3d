...
<#
.SYNOPSIS
    Alfr3d installer & management script for Windows.
.DESCRIPTION
    One-liner install:
      irm https://cdn.link-ai.tech/code/alfr3d/run.ps1 | iex
    Or from a local clone:
      .\scripts\run.ps1              # install / configure
      .\scripts\run.ps1 start        # start service  (delegates to alfr3d CLI)
...
# ── colours ──────────────────────────────────────────────────────
function Write-Alfr3d   { param([string]$M) Write-Host $M -ForegroundColor Green  }
...
# Initialize $UiLang for management commands: prefer alfr3d_lang from an existing
...
function Initialize-UiLang {
    ...
    if (Test-Path "$BaseDir\config.json") {
        try {
            ...
            if ($cfg.alfr3d_lang) { $cfgLang = "$($cfg.alfr3d_lang)" }
        } catch {}
    }
    switch ($cfgLang) {
        "zh"    { $script:UiLang = "zh" }
...
function Assert-Python {
    ...
    Write-Alfr3d ((T "找到 Python" "Found Python") + ": $PythonCmd")
}
...
function Install-Project {
    if (Test-Path "Alfr3d") {
        ...
        $backup = "Alfr3d_backup_$(Get-Date -Format 'yyyyMMddHHmmss')"
        Rename-Item "Alfr3d" $backup
        Write-Warn ((T "已存在 Alfr3d 目录，已自动备份为" "Existing 'Alfr3d' directory backed up to") + " '$backup'")
    }
    ...
    Write-Alfr3d (T "正在克隆 Alfr3d 项目..." "Cloning Alfr3d project...")
    $cloneOk = $false
    ...
    try {
        ...
        Write-Alfr3d (T "GitHub 可达，正在从 GitHub 克隆..." "GitHub is reachable, cloning from GitHub...")
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        git clone --depth 10 --progress "https://github.com/NewSoulOnTheBlock/alfr3d.git" 2>&1 | ForEach-Object { Write-Host $_ }
        if ($LASTEXITCODE -eq 0) { $cloneOk = $true }
        $ErrorActionPreference = $prevEAP
        if (-not $cloneOk) {
            if (Test-Path "Alfr3d") { Remove-Item -Recurse -Force "Alfr3d" }
        }
    } catch {}
    ...
    if (-not $cloneOk) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        git clone --depth 10 --progress "https://gitee.com/NewSoulOnTheBlock/alfr3d.git" 2>&1 | ForEach-Object { Write-Host $_ }
        if ($LASTEXITCODE -eq 0) { $cloneOk = $true }
        $ErrorActionPreference = $prevEAP
        if (-not $cloneOk) {
            if (Test-Path "Alfr3d") { Remove-Item -Recurse -Force "Alfr3d" }
    ...
    if (-not $cloneOk) {
        ...
        Write-Err (T "你也可以手动克隆: git clone https://gitee.com/NewSoulOnTheBlock/alfr3d.git" "You can also manually clone: git clone https://gitee.com/NewSoulOnTheBlock/alfr3d.git")
        Read-Host (T "按回车退出" "Press Enter to exit")
        exit 1
    }
    ...
    Write-Alfr3d ((T "项目已克隆" "Project cloned") + ": $BaseDir")
}
...
function Install-Dependencies {
    Write-Alfr3d (T "正在安装依赖..." "Installing dependencies...")
    ...
    Write-Alfr3d (T "正在注册 alfr3d CLI..." "Registering alfr3d CLI...")
    $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    & $PythonCmd -m pip install -e $BaseDir @pipMirror 2>&1 | Out-Null
    $ErrorActionPreference = $prevEAP

    # Add the Scripts dir (where pip puts alfr3d.exe) to PATH, both for this
    # session and persistently, so `alfr3d` keeps working after a reboot.
    $scriptsDir = & $PythonCmd -c "import sysconfig; print(sysconfig.get_path('scripts'))" 2>$null
    if ($scriptsDir -and (Test-Path $scriptsDir)) {
        Add-ScriptsDirToPath $scriptsDir
    }

    $alfr3dBin = Get-Command alfr3d -ErrorAction SilentlyContinue
    if ($alfr3dBin) {
        Write-Alfr3d ((T "alfr3d CLI 注册成功" "alfr3d CLI registered") + ": $($alfr3dBin.Source)")
    } else {
        Write-Warn ((T "alfr3d CLI 不在 PATH 中，你可以使用" "alfr3d CLI not in PATH. You can use") + ": $PythonCmd -m cli.cli")
    }
...
function Install-Ripgrep {
    ...
    try {
        ...
        if (Get-Command rg -ErrorAction SilentlyContinue) {
            Write-Alfr3d (T "已检测到 ripgrep (rg)，跳过安装" "ripgrep (rg) already available, skipping")
            return
        }
        ...
        Write-Alfr3d (T "正在安装 ripgrep（加速文件搜索）..." "Installing ripgrep (faster file search)...")
        ...
        $urls = @(
            ...
            "https://cdn.link-ai.tech/code/alfr3d/vendor/$asset"
        )

        $tmpZip = Join-Path $env:TEMP "alfr3d-$asset"
        $tmpDir = Join-Path $env:TEMP "alfr3d-rg-$([guid]::NewGuid().ToString('N'))"
        $downloaded = $false
        foreach ($u in $urls) {
            try {
                $oldPP = $ProgressPreference; $ProgressPreference = "SilentlyContinue"
        ...
        if ($rgExe) {
            ...
            Write-Alfr3d ((T "ripgrep 安装成功" "ripgrep installed") + ": $rgTarget")
        } else {
...
# user PATH (User scope needs no admin rights), so `alfr3d` survives a reboot.
function Add-ScriptsDirToPath {
    ...
    try {
        ...
        if (-not $already) {
            ...
            Write-Alfr3d ((T "已将 alfr3d 命令目录加入用户 PATH（重启后仍可用）" "Added alfr3d command directory to user PATH (persists after reboot)") + ": $ScriptsDir")
        }
    } catch {
...
function Configure-Model {
    ...
    Write-Alfr3d ((T "正在配置" "Configuring") + " $($m.Provider)...")
...
function Configure-Channel {
    ...
    switch ($script:ChannelChoice) {
        ...
        "feishu" {
            ...
            Write-Alfr3d (T "配置飞书（WebSocket 模式）..." "Configure Feishu (WebSocket mode)...")
        ...
        "dingtalk" {
            ...
            Write-Alfr3d (T "配置钉钉..." "Configure DingTalk...")
        ...
        "wecom_bot" {
            ...
            Write-Alfr3d (T "配置企微智能机器人..." "Configure WeCom Bot...")
        ...
        "qq" {
            ...
            Write-Alfr3d (T "配置 QQ 机器人..." "Configure QQ Bot...")
        ...
        "wechatcom_app" {
            ...
            Write-Alfr3d (T "配置企微自建应用..." "Configure WeCom App...")
        ...
        "telegram" {
            ...
            Write-Alfr3d (T "配置 Telegram..." "Configure Telegram...")
        ...
        "slack" {
            ...
            Write-Alfr3d (T "配置 Slack..." "Configure Slack...")
        ...
        "discord" {
            ...
            Write-Alfr3d (T "配置 Discord..." "Configure Discord...")
...
function New-ConfigFile {
    Write-Alfr3d (T "正在生成 config.json..." "Generating config.json...")

    $config = [ordered]@{
        channel_type              = if ($script:ChannelType) { $script:ChannelType } else { "web" }
        model                     = if ($script:ModelName)  { $script:ModelName }  else { "" }
        alfr3d_lang                  = if ($script:InstallLang) { $script:InstallLang } else { "auto" }
        open_ai_api_key           = ""
    ...
    Write-Alfr3d (T "配置文件创建成功。" "Configuration file created.")
}

# Resolve the `alfr3d` command, self-healing PATH if needed. When `alfr3d` is missing
...
function Resolve-Alfr3dCommand {
    if (Get-Command alfr3d -ErrorAction SilentlyContinue) { return $true }

    $py = if ($PythonCmd) { $PythonCmd } else { Find-Python }
    if ($py) {
        $scriptsDir = & $py -c "import sysconfig; print(sysconfig.get_path('scripts'))" 2>$null
        if ($scriptsDir -and (Test-Path "$scriptsDir\alfr3d.exe")) {
            Add-ScriptsDirToPath $scriptsDir
            if (Get-Command alfr3d -ErrorAction SilentlyContinue) { return $true }
        }
    }
...
# ── start via alfr3d CLI ─────────────────────────────────────────────
function Start-Alfr3d {
    Write-Alfr3d (T "正在启动 Alfr3d..." "Starting Alfr3d...")
    if (Resolve-Alfr3dCommand) {
        & alfr3d start
    } else {
        Write-Warn (T "未找到 alfr3d CLI，直接启动..." "alfr3d CLI not found, starting directly...")
        & $PythonCmd "$BaseDir\app.py"
    }
}

# ── delegate management commands to alfr3d CLI ──────────────────────
function Invoke-Alfr3dCommand {
    param([string]$Cmd)
    if (Resolve-Alfr3dCommand) {
        & alfr3d $Cmd
    } else {
        # Fall back to the module entrypoint so management commands still work
        # even when alfr3d.exe isn't on PATH (e.g. right after a fresh reboot on
        # an older install).
        $py = if ($PythonCmd) { $PythonCmd } else { Find-Python }
        if ($py -and (Test-Path "$BaseDir\app.py")) {
            Write-Warn (T "未找到 alfr3d 命令，使用 python -m cli.cli 兜底运行..." "alfr3d command not found, falling back to python -m cli.cli...")
            Push-Location $BaseDir
            try { & $py -m cli.cli $Cmd } finally { Pop-Location }
        } else {
            Write-Err (T "未找到 alfr3d CLI，请先不带参数运行本脚本进行安装。" "alfr3d CLI not found. Run this script without arguments first to install.")
            exit 1
        }
    }
}
...
function Show-Usage {
    Write-Info "========================================="
    Write-Info "   Alfr3d Management Script (Windows)"
    Write-Info "========================================="
    Write-Host ""
...
function Install-Mode {
    Clear-Host
    Write-Info "========================================="
    Write-Info "   Alfr3d Installation (Windows)"
    Write-Info "========================================="
    Write-Host ""
    ...
    if ($IsProjectDir) {
        Write-Alfr3d (T "检测到已有项目目录。" "Detected existing project directory.")
        if (Test-Path "$BaseDir\config.json") {
            Write-Alfr3d (T "项目已配置。" "Project already configured.")
            Write-Host ""
            Show-Usage
            return
        }
    ...
    Write-Host ""
    if ($script:AccessInfo) { Write-Alfr3d $script:AccessInfo }
    Write-Warn (T "提示：需要让 Agent 浏览网页时，运行 alfr3d install-browser 安装浏览器工具" "Tip: to let the Agent browse the web, run 'alfr3d install-browser' to install the browser tool")
    Start-Alfr3d
}

# ── update ────────────────────────────────────────────────────────
function Update-Project {
    Write-Alfr3d (T "正在更新 Alfr3d..." "Updating Alfr3d...")
    ...
    if ($alfr3dBin) {
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        & alfr3d stop 2>&1 | Out-Null
        $ErrorActionPreference = $prevEAP
    }

    if (Test-Path "$BaseDir\.git") {
        Write-Alfr3d (T "正在拉取最新代码..." "Pulling latest code...")
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        ...
        if ($pullExit -ne 0) {
            ...
            git remote set-url origin https://gitee.com/NewSoulOnTheBlock/alfr3d.git 2>&1 | Out-Null
            git pull 2>&1 | Out-Null
            $ErrorActionPreference = $prevEAP
        }
    } else {
    ...
    # Start via python -m cli.cli instead of alfr3d.exe, because the exe may
...
switch ($Command.ToLower()) {
    ""        { Install-Mode }
    "start"   { Invoke-Alfr3dCommand "start" }
    "stop"    { Invoke-Alfr3dCommand "stop" }
    "restart" { Invoke-Alfr3dCommand "restart" }
    "status"  { Invoke-Alfr3dCommand "status" }
    "logs"    { Invoke-Alfr3dCommand "logs" }
    "config"  {
        Assert-Python
        Install-Dependencies
        Select-Model
        ...
        if ($r -ne "n" -and $r -ne "N") { Invoke-Alfr3dCommand "restart" }
    }
